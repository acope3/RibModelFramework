...
\label{eq:distYgCodon}&= \frac{\Gamma\left(\alphacgprime + \Ygc\right)}{\Gamma\left(\alphacgprime\right) \Ygc!} 
  \left(\frac{\phig}{\lambdacprime + \phig}\right)^\Ygc \left(\frac{\lambdacprime}{\lambdacprime + \phig}\right)^{\alphacgprime}
\end{align}
...
Thus to simulate \Yg values based on equations (\ref{eq:distYgSite}) or (\ref{eq:distYgCodon}), first pull $W$ from Gamma(\alphac, \lambdacprime), then pull $Y$ from Poisson($W \ngc \phig$).
...
First, we generate $W_c$  from $\text{Gamma}(\alphac, \lambdacprime)$ and then we generate $\Ygc$ from $\text{Poisson}\left(\phi_g W_c\right)$
...
\end{align}
where $E_p(z)$ is the generalized exponential integral function and can is represented as,
\begin{equation}
  \label{eq:defGeneralizedExpoInt}
   E_p(z) = \int_1^\infty \frac{e^{-z t}}{t^p} = z^{p-1} \int_z^\infty \frac{e^{-t}}{t^p} dt = z^{p-1} \Gamma(1-p,z)
\end{equation}
where $\Gamma(1-p,z)$ is the incomplete gamma function.
