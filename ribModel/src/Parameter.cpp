...
void Parameter::setNumMutationSelectionValues(std::string mutationSelectionState, std::vector<std::vector<unsigned>> thetaKMatrix)
{
...
void Parameter::initCategoryDefinitions(std::string mutationSelectionState, std::vector<std::vector<unsigned>> thetaKMatrix)
{
	std::set<unsigned> delMCounter;
	std::set<unsigned> delEtaCounter;
...
void Parameter::InitializeSynthesisRate(Genome& genome, double sd_phi)
{
	...
	for(unsigned category = 0u; category < numSelectionCategories; category++)
	{
		...
		{
			currentSynthesisRateLevel[category][j] = expression[index[j]];
