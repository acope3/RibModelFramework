...
plotSinglePanel <- function(parameter, model, genome, expressionValues, samples, mixture, aa)
{
  ...
  {
    selection[i] <- parameter$getCodonSpecificPosteriorMean(codons[i],mixture, samples, 1, T)
    mutation[i] <- parameter$getCodonSpecificPosteriorMean(codons[i],mixture, samples, 0, T)
  }
