  ...
  if (class(parameter) == "Rcpp_ROCParameter" || class(parameter) == "Rcpp_FONSEParameter"){
    ...
    for(aa in names.aa){
      ...
      for(i in 1:length(codons)){
        ...
        if(CSP == "Mutation"){
          ...
          quantile_list <- c(quantile_list, parameter$getCodonSpecificQuantile(mixture, samples, codons[i], 0, c(0.025, 0.975), TRUE, FALSE))
        }
        else if(CSP == "Selection"){
          Value <- c(Value, parameter$getCodonSpecificPosteriorMean(mixture, samples, codons[i], 1, TRUE))
          quantile_list <- c(quantile_list, parameter$getCodonSpecificQuantile(mixture, samples, codons[i], 1, c(0.025, 0.975), TRUE,FALSE))
        }
        else {
    ...
    for(i in 1:length(groupList)){
      ...
      if(CSP == "Alpha"){
        ...
        quantile_list <- c(quantile_list, parameter$getCodonSpecificQuantile(mixture, samples, codons[i], 0, c(0.025, 0.975), FALSE,FALSE))
        }
      else if(CSP == "Lambda Prime"){
        Value <- c(Value, parameter$getCodonSpecificPosteriorMean(mixture, samples, codons[i], 1, FALSE))
        quantile_list <- c(quantile_list, parameter$getCodonSpecificQuantile(mixture, samples, codons[i], 1, c(0.025, 0.975), FALSE,FALSE))
      }
      else {
        stop("Unknown parameter type given with argument: CSP")
      }
