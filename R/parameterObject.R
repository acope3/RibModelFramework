...
#' #' If passed in as single number (default is 0), this will be the standard deviation value for all categories, for all codons. User may also
#' supply a vector with n * 40 values, where n is the number of mutation categories. Future versions will check the number of rows matches
#' the number of mutation categories definded by user. 
#' 
...
#' }
#'  
updated.marginal.log.likelihood<- function(parameter, mcmc, n.samples, divisor,est.CSP=T,est.SynthesisRate=T)
{
  ...
  mut.cateogries <- unique(categories[[1]])
  sel.categories <- unique(categories[[2]])
  if (est.CSP)
  {
    for (mixture in mut.cateogries)
    {
      for(aa in AnaCoDa::aminoAcids()) # for all amino acids
      {
        ...
        {
          vec <- trace$getCodonSpecificParameterTraceByMixtureElementForCodon(mixture, codons[i], 0, TRUE)
    ...
    for (mixture in sel.cateogries)
    {
      for(aa in AnaCoDa::aminoAcids()) # for all amino acids
      {
        ...
        for(i in 1:length(codons)) # for all codons
        {
          vec <- trace$getCodonSpecificParameterTraceByMixtureElementForCodon(mixture, codons[i], 1, TRUE)
          vec <- vec[(length(vec) - n.samples+1):(length(vec))]
          sample_mat[,i] <- vec
          mean_vals[i] <- mean(vec)
        }
  ...
  if (est.SynthesisRate)
  {
    for (mixture in sel.categories)
    {
